...
static void adder(void * unusedpointer, unsigned long addernumber)
{
        ...
        while (flag) {
                /* loop doing increments until we achieve the overall number
                   of increments */
                /* acquire the counter lock to ensure mutually exclusive access to critical section
                   where counter is being modified
                 */
...
int maths (int data1, char **data2)
{
         ...
         */
        /* create counter lock */
        count_lock = lock_create("counter lock");
        if (count_lock == NULL) panic("Lock could not be created!");
