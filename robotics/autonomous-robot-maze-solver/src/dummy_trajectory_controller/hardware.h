 ...
 */
using encoder_count = long long int;

struct encoder_monitor {
    bool last_int_state;
    hardware::drive_direction direction;
    long long int count;

    encoder_monitor() :
        last_int_state{0},
        direction{hardware::drive_direction::forward},
        count{0}
    {
        // nop
    }
...
class encoder
{
    ...
    // static auto set_stopped (bool stopped) -> void;

    // static auto get_velocity() -> double*;

    static auto get_count() -> long long int;

    static auto interrupt_callback() -> void;

    private:
        static encoder_monitor monitor_;
...
class lidar
{
    ...
    static auto distance () -> units::millimeters {
        Serial.print("enter lidar ");
        Serial.print_line(tag);
...
class serial_api
{
     ...
     */
    static auto print (const __FlashStringHelper* string) -> char_count;
    static auto print (char const * string) -> char_count;
    
    template <typename T, typename... Args>
    static auto print_vector (T first, Args... args) -> char_count;

    template <typename T>
    static auto print_vector (const T& first) -> char_count;
     ...
     * \return total number of bytes written.
     */
    static auto print_line (const __FlashStringHelper* string) -> char_count;
...
constexpr bool to_bool(hardware::logic_level level) {
    return (level == hardware::logic_level::high);
}

constexpr hardware::logic_level to_logic_level(bool level) {
    return level ? hardware::logic_level::high : hardware::logic_level::low;
}

namespace locomotion {

uint8_t to_mode(analog_reference ref);
double to_range(analog_reference ref);
