...
f1 = figure;
hdl_fit = plot(0,0); xlabel('Generations'); ylabel('Fitness value');

figure;
hdl_hub = scatter(0,0, 'ro','MarkerFaceColor','r'); xlabel('X'); ylabel('Y');
hold on; scatter(towns(:,1),towns(:,2), 'k.');grid on;
text(towns(:,1)+1,towns(:,2)+1,string(1:size(towns,1)));
hdl_t_to_hub = plot(0,0,'b-');
hdl_hub_to_hub = plot(0,0,'g-');
% For each generation
for g=1:G,
    ...
    disp(length(best_cost_history));
    disp(best_cost_history);
    set(hdl_fit,'xdata',1:length(best_cost_history),'ydata',best_cost_history);
    m = best_chromosome;
    x = find(m);
    set(hdl_hub,'xdata',towns(x,1),'ydata',towns(x,2));
    ...
    set(hdl_t_to_hub,'xdata',towns(l,1),'ydata',towns(l,2));
